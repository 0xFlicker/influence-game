...
export function decodeVisualComposition(text: string, playerIds: readonly string[]): VisualLocalization {
  ...
  if (value.count !== playerIds.length || new Set(value.identities.map((entry) => entry.playerId)).size !== playerIds.length || value.identities.some((entry) => entry.confidence !== "clear")) throw new VisualIdentityFailure("Scene participants are missing, duplicated or ambiguous");
