...
describe("sealed provider manifest migration", () => {
  ...
  test.each([
    ...
    [
      "empty manifest",
      '{"modelSelection":{"catalogId":"openai:gpt-5-nano"},"providerManifest":[]}',
    ],
    [
    ...
    "rejects %s without partially backfilling other games",
    async (_label, invalidConfig) => {
      const db = await setupTestDB();
    ...
    `),
      ...
      try {
        ...
        expect(await readConfigText(db, testSchema)).toEqual({
          ...
          valid:
            '{"modelSelection":{"catalogId":"openai:gpt-5-nano","reasoningPolicy":"low"}}',
        });
      } finally {
        await db.execute(sql.raw(`DROP SCHEMA "${testSchema}" CASCADE`));
      }
...
async function readConfigs(
  ...
  const rows = await db.execute<{ id: string; config: string }>(
    sql.raw(`
    SELECT "id", "config" FROM "${testSchema}"."games" ORDER BY "id"
  `),
  );
  return Object.fromEntries(
    [...rows].map((row) => [row.id, JSON.parse(row.config)]),
  );
}
