...
describe("Game REST API", () => {
  ...
  describe("POST /api/games", () => {
    ...
    test("normalizes explicit standard service tier and rejects invalid values", async () => {
      const standardRes = await app.request(
        "/api/games",
        json({ playerCount: 6, serviceTier: "standard" }, adminToken),
      );

      expect(standardRes.status).toBe(201);
      const standardBody = (await standardRes.json()) as { id: string };
      const game = (await db
        .select()
        .from(schema.games)
        .where(eq(schema.games.id, standardBody.id)))[0]!;
      ...
      expect(await invalidRes.json()).toEqual({ error: "Invalid service tier" });
    });
