...
const RELEASE_MIGRATION_RULES: Array<{
  ...
  { rule: "rename-column", pattern: /\bALTER\s+TABLE\b[\s\S]*?\bRENAME\s+COLUMN\b/i },
  { rule: "rename-table", pattern: /\bALTER\s+TABLE\b[\s\S]*?\bRENAME\s+TO\b/i },
  { rule: "narrow-type", pattern: /\bALTER\s+TABLE\b[\s\S]*?\bALTER\s+COLUMN\b[\s\S]*?\bTYPE\b/i },
  { rule: "set-not-null", pattern: /\bALTER\s+TABLE\b[\s\S]*?\bALTER\s+COLUMN\b[\s\S]*?\bSET\s+NOT\s+NULL\b/i },
  { rule: "drop-default", pattern: /\bALTER\s+TABLE\b[\s\S]*?\bALTER\s+COLUMN\b[\s\S]*?\bDROP\s+DEFAULT\b/i },
...
export function resolveMigrationsFolder(
  ...
  return env.DRIZZLE_MIGRATIONS_DIR
...
export function calculateMigrationSet(
  ...
  const records = files.map((file) => {
    ...
    const relative = path.relative(migrationsFolder, file).split(path.sep).join("/");
...
if (import.meta.main) {
  ...
  if (command === "--print-release-migration-set") {
    ...
    console.log(`Release migration policy accepted ${files.length} changed migration file(s).`);
  } else if (command) {
