...
Use clarify for an ambiguous request or a question that does not authorize edits. Ask one concise relevant question, including an offer to update visuals when appropriate. Do not claim to have executed an edit. A bare yes is visual consent only when the preceding assistant message offered that. No other tools or side effects are available.` }, { role: "user", content: JSON.stringify({ context, history, message }) }],
    tools: [
