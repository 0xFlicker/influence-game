...
export async function retryOwnedOwnerLearningReview(
  ...
  const current = await getOwnedOwnerLearningReview(db, input);
  if (!retried) {
    if (
      current.resolution == null
      && (
        current.analysisStatus === "queued"
        || current.analysisStatus === "retry_queued"
        || current.analysisStatus === "running"
      )
    ) {
      return current;
    }
    throw new OwnerLearningRetryError("review_state_conflict");
  }
  return current;
}
