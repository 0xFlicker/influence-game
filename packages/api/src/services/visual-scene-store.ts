...
export async function prepareVisualScene(db: DrizzleDB, input: {
  gameId: string; boundarySequence: number; plan: VisualScenePlan;
  ...
  return db.transaction(async (tx) => {
    ...
    const referenceIds = [...new Set([input.plan.backgroundArtifactId, ...input.plan.cast.map((member) => member.referenceArtifactId)])];
    ...
    const [scene] = await tx.insert(scenes).values({
      id: randomUUID(), gameId: input.gameId, roomId: input.plan.roomId, boundarySequence: input.boundarySequence, plan: input.plan, planHash,
...
export async function acceptVisualScene(db: DrizzleDB, input: {
  sceneId: string; planHash: string; imageArtifactId: string; anchors: readonly VisualPlayerAnchor[];
}): Promise<StoredVisualScene> {
  ...
  assertVisualAnchors(input.anchors, planned.plan.cast.map((member) => member.id));
  ...
  return db.transaction(async (tx) => {
    ...
    if (!latest || latest.id !== planned.id) throw new Error("Stale visual scene result");
...
export async function failVisualScene(db: DrizzleDB, sceneId: string, failure: string): Promise<void> {
  if (!failure.trim()) throw new Error("Visual failure requires an explanation");
  await db.update(scenes).set({ status: "failed", failure }).where(and(eq(scenes.id, sceneId), eq(scenes.status, "preparing")));
}
