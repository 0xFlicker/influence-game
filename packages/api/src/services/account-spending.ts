...
const totals = sql`count(DISTINCT operation_id) FILTER (WHERE kind IN ('creation_assistant','profile_creation','profile_refinement'))::int AS text_requests,
 ...
 coalesce(sum(actual),0)::float8 AS actual_microusd, coalesce(sum(estimated),0)::float8 AS estimated_microusd,
...
export async function readAccountSpending(db: DrizzleDB, actor: string, query: Record<string, string>) {
  ...
  const sorts: Record<string, ReturnType<typeof sql>> = { spend: sql`(actual_microusd + estimated_microusd)`, text: sql`text_requests`, images: sql`images`, attempts: sql`attempts`, failures: sql`failures`, activity: sql`last_activity` };
 ...
 SELECT u.id, coalesce(u.display_name,u.handle,u.email,u.wallet_address,u.id) AS label,
 coalesce(t.text_requests,0) AS text_requests, coalesce(t.images,0) AS images, coalesce(t.attempts,0) AS attempts,
 coalesce(t.failures,0) AS failures, coalesce(t.actual_microusd,0) AS actual_microusd, coalesce(t.estimated_microusd,0) AS estimated_microusd,
