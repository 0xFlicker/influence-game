...
export async function renderPlannedVisualScene(db: DrizzleDB, scene: StoredVisualScene, signal?: AbortSignal, assertBoundary?: VisualBoundaryGuard): Promise<StoredVisualScene> {
  ...
  try {
    ...
    const renderKey = `${scene.id}:render:${scene.renderRevision}`;
    ...
    for (const [index, group] of groups.entries()) {
      ...
      const rendered = await renderDurableVisualImage(db, { gameId, sceneId: scene.id, operationKey: `${renderKey}:section:v2:${index}`, allowFallback: scene.renderRevision === 0, request: { prompt, width, height, references: [...(background ? [background] : []), ...members.map((member) => member.image)] }, signal, beforeDispatch });
    ...
    if (sectionImages.length > 1) {
      ...
      const rendered = await renderDurableVisualImage(db, { gameId, sceneId: scene.id, operationKey: `${renderKey}:harmonize`, allowFallback: scene.renderRevision === 0, signal, beforeDispatch,
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.update(schema.visualScenes).set({ candidateArtifactId: imageArtifactId }).where(and(eq(schema.visualScenes.id, scene.id), eq(schema.visualScenes.renderRevision, scene.renderRevision), eq(schema.visualScenes.status, "preparing")));
    });
    const localization = await localizeDurableVisualScene(db, { gameId, sceneId: scene.id, operationKey: `${renderKey}:localization:${VISUAL_LOCALIZATION_VERSION}`, scene: finalImage,
