...
beforeAll(async () => {
  ...
  web = Bun.spawn(["bun", "run", "dev", "--hostname", "127.0.0.1"], {
    ...
    stdout: Bun.file("/private/tmp/public-creator-browser-web.log"), stderr: Bun.file("/private/tmp/public-creator-browser-web-errors.log"),
  });
  ...
  while (true) {
    ...
    if (Date.now() >= deadline) throw new Error("Public creator web server did not start; see /private/tmp/public-creator-browser-web-errors.log");
    await Bun.sleep(250);
  }
...
test("public hall → one tracked preview → image signup → free account messages, with draft preserved", async () => {
  ...
  try {
    ...
    await page.screenshot({ path: "/private/tmp/public-creator-desktop.png" });
    ...
    await page.screenshot({ path: "/private/tmp/public-creator-signup-desktop.png" });
    await page.setViewport({ width: 390, height: 844 });
    await page.screenshot({ path: "/private/tmp/public-creator-signup-mobile.png" });
    ...
    expect(await database.db.select().from(schema.anonymousTextOperations)).toHaveLength(1);
    await page.screenshot({ path: "/private/tmp/public-creator-after-signup-mobile.png" });
  } catch (error) {
    await page.screenshot({ path: "/private/tmp/public-creator-browser-failure.png" });
