...
export function OwnerLearningEntryView({
  ...
  const visiblePreflight = preflightPending ? null : preflight;
  ...
  const startState = startAvailability(
    eligible,
    preflight,
    selectedGameIds,
    preflightPending,
    ...
    <div className="olm-enter" data-testid="owner-learning-entry">
      ...
      <div className="olm-entry-hero">
        ...
        <aside className="olm-decision-panel" aria-label="Start agent review">
          ...
          <p
            className="olm-decision-status"
            role="status"
            aria-live="polite"
            aria-busy={startState.loadingEvidence}
          >
            {startState.detail}
          </p>
          ...
          <div className="olm-button-row">
            ...
            >
              ...
              {startState.loadingEvidence
                ? (
                  <span
                    className="olm-button-icon olm-button-spinner animate-spin motion-reduce:animate-none"
                    aria-hidden="true"
                  />
                )
                : <span className="olm-button-icon" aria-hidden="true">→</span>}
            </button>
            {onDismiss && (
      ...
      <section className="olm-section" aria-labelledby="olm-games-title">
        ...
        <div className="olm-filmstrip">
          {profile.games.map((game) => {
            ...
            const projected = selectedPreflightGame(visiblePreflight, game.gameId);
      ...
      {visiblePreflight?.status === "awaiting_evidence" && (
      ...
      {visiblePreflight?.evidence.analysisTrack === "strategy_health_check" && (
...
function startAvailability(
  ...
  if (pending) {
    return {
      available: false,
      detail: "Loading the accepted actions and counterplay first.",
  ...
  if (!preflight) {
    return failed
      ? {
        available: false,
        detail: "Evidence loading stopped. Change the selected games to try again.",
        loadingEvidence: false,
      }
      : {
        available: false,
  ...
  if (preflight.status === "generation_unavailable") {
    return {
      available: false,
      detail: "Strategic review is temporarily unavailable. Your credit has not been used.",
      loadingEvidence: false,
    };
  }
  if (preflight.status === "awaiting_evidence") {
    return {
      available: false,
      detail: "The selected early exits do not yet contain enough evidence for paid analysis.",
      loadingEvidence: false,
    };
  }
  return eligible.credit.mode === "unlimited"
    ? {
      available: true,
      detail: "Recorded actions and counterplay are ready. Starting opens the private sysop analysis.",
      loadingEvidence: false,
    }
    : {
      available: true,
      detail: "Recorded actions and counterplay are ready. Purchasing starts the private analysis.",
      loadingEvidence: false,
    };
}
