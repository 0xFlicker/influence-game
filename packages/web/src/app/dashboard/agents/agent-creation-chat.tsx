...
export function AgentCreationChat({ profile, onGenerate, onAppearance, avatarUrl, busy, blocked, headRequired, hasImage, onHeadshot, onAdvanced, onCancel, onSaveDraft, submitDisabled, submitLabel, creationTraitIds = [], onCreationTraitIdsChange }: {
  ...
  return <>
    ...
    <section aria-label="Character fixtures" className="min-h-0 flex-1 overflow-auto bg-white/[.025] px-4 py-3 sm:px-8">
      {working && (stage === "appearance" || stage === "portrait") ? <CharacterFormation name={profile.name} /> : <div className="mx-auto flex max-w-5xl items-start gap-4">
        ...
        <div className="grid min-w-0 flex-1 grid-cols-2 gap-2 sm:grid-cols-3">
          ...
          {stage === "character" && !profile.personality && <p className="col-span-2 py-3 text-sm text-white/45">Your name, archetype, and character prompt will take shape here.</p>}
        </div>
      </div>}
    </section>
    <footer className="shrink-0 bg-[#11111b] px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 sm:px-8">
      <div className="mx-auto max-w-2xl space-y-3">
        ...
        {!ended && showIngredients && onCreationTraitIdsChange && <div hidden={working}><AgentCreationIngredients key={ingredientMode} mode={ingredientMode} selected={creationTraitIds} onChange={onCreationTraitIdsChange} disabled={locked} /></div>}
        ...
        {!ended && <div className={working ? "relative h-12" : "relative"}>
          <textarea hidden={working} ref={composer} aria-label="Message the character assistant" placeholder={stage === "appearance" || stage === "portrait" ? "Describe their look…" : "Tell me about your character…"} value={input} maxLength={1400} disabled={locked} onInput={event => setInput(event.currentTarget.value)} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void send(); } }} rows={2} className="influence-field max-h-32 min-h-16 w-full resize-none rounded-2xl py-3 pl-3 pr-14 text-base" />
          <button type="button" aria-label={working ? "Assistant working" : "Send"} aria-busy={working} disabled={locked || (!input.trim() && !hasIngredients)} onClick={() => void send()} className={`absolute bottom-2 right-2 flex size-10 items-center justify-center rounded-full bg-violet-400 text-[#11111b] transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-violet-300 ${working ? "opacity-100" : "hover:bg-violet-300 disabled:opacity-35"}`}>
            {working ? <span aria-hidden="true" className="size-5 animate-spin rounded-full border-2 border-current/25 border-t-current motion-reduce:animate-none" /> : <svg aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 19V5m-6 6 6-6 6 6" /></svg>}
          </button>
        </div>}
