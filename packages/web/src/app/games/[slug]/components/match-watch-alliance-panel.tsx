    ...
    <section className="rounded-md border border-white/10 bg-white/[0.02] p-3">
      ...
      {allianceModel.status === "ready" ? (
        <>
          ...
          />
          <AllianceCardList cards={allianceModel.cards} />
        </>
      ) : (
...
export function AllianceCardList({
  ...
  return (
    <div className="space-y-3">
      {cards.map((card) => (
        <AllianceCard key={card.id} card={card} compact={compact} />
      ))}
    </div>
  );
}
...
function AllianceMemberAvatars({ card }: { card: MatchWatchAllianceCardModel }) {
  return (
