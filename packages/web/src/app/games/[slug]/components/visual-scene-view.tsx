...
export function VisualSceneView({ scene, speech, elapsedMs, clockElapsedMs = elapsedMs, reducedMotion = false, navigationRevision = 0, controlsInset = 0 }: {
  scene: AcceptedVisualScene; speech: VisualSpeech | null; elapsedMs: number;
  controlsInset?: number; clockElapsedMs?: number; reducedMotion?: boolean; navigationRevision?: number;
}) {
  ...
  const framing = reducedMotion ? target : panScene(camera.from, target, clockElapsedMs);
  ...
  return <section ref={frameRef} aria-label="Current room" className="relative min-h-0 w-full flex-1 overflow-hidden">
    ...
    {speech && opacity > 0 && <div className="absolute z-10 flex flex-col rounded-2xl border border-white/20 bg-black/90 px-4 py-3 text-white shadow-xl"
      ...
      <TimedSpeech text={speech.text} elapsedMs={clockElapsedMs} className="text-sm leading-relaxed" />
