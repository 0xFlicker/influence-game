...
export function soloPresentationMotion(text: string, elapsedMs: number, paused = false, reducedMotion = false) {
  ...
  // Paused seeks show the line immediately. Pausing an active fade freezes it.
  const time = paused && elapsedMs === 0 ? SOLO_SPEECH_START_MS + SOLO_SPEECH_FADE_MS : elapsedMs;
