...
export function GameViewer({
  ...
  const [recentlyUnshielded, setRecentlyUnshielded] = useState<
    ReadonlySet<string>
  >(new Set());
  ...
  const [spectacleCurrent, setSpectacleCurrent] =
    ...
    (ev: WsGameEvent) => {
      switch (ev.type) {
        case "watch_state": {
          ...
          if (newlyUnshielded.length > 0) {
            setRecentlyUnshielded(
              (prev) => new Set([...prev, ...newlyUnshielded]),
            );
        ...
        case "phase_change": {
          ...
          if (
            (prevPhase === "REVEAL" || prevPhase === "COUNCIL") &&
            ev.phase !== "REVEAL" &&
            ev.phase !== "COUNCIL"
          ) {
            setRevealQueue((q) => {
          ...
          if (ev.phase === "INTRODUCTION") audioCue.zone("ambient");
            ...
            ev.phase === "MINGLE_I" ||
            ev.phase === "PRE_VOTE_HUDDLE" ||
            ev.phase === "MINGLE" ||
            ev.phase === "POST_VOTE_MINGLE" ||
            ev.phase === "VOTE" ||
            ev.phase === "PRE_COUNCIL_HUDDLE"
          )
        ...
        case "message": {
          ...
          // Mobile badge counts
          if (ev.entry.scope === "public" && mobileTabRef.current !== "chat") {
  ...
  const visibleMessages = isReplay
  ...
  const replayGame: GameDetail = isReplay
    ? {
        ...game,
        currentPhase: (visibleMessages.findLast((m) => m.phase)?.phase ??
          game.currentPhase) as PhaseKey,
  ...
  const currentWhisperEntries = visibleMessages.filter(
    (message) =>
      (message.phase === "MINGLE" || message.phase === "POST_VOTE_MINGLE") &&
  ...
  return (
    <>
      ...
      <div className="md:hidden flex flex-col h-[calc(100dvh-4rem)] pb-16 overflow-hidden">
        ...
        {mobileTab === "chat" &&
          (replayGame.currentPhase === "INTRODUCTION" ||
            replayGame.currentPhase === "LOBBY") &&
          !isReplay && (
            <GroupChatFeed
        ...
        {mobileTab === "chat" &&
          (replayGame.currentPhase === "MINGLE" ||
            replayGame.currentPhase === "POST_VOTE_MINGLE") &&
          !isReplay && (
            <WhisperPhaseView
        ...
        {mobileTab === "chat" &&
          replayGame.currentPhase === "DIARY_ROOM" &&
          !isReplay && (
            <DiaryRoomGridView
        ...
        {mobileTab === "chat" &&
          replayGame.currentPhase === "JURY_QUESTIONS" &&
          !isReplay && (
            <JuryDMView messages={currentJuryMessages} players={game.players} />
          )}
        {mobileTab === "chat" &&
        ...
        {mobileTab === "chat" && isReplay && (
          <div ref={feedRef} className="flex-1 overflow-y-auto p-4 space-y-3">
            {visibleMessages.filter(
              (m) => m.scope !== "diary" && m.scope !== "whisper",
            ).length === 0 ? (
              <p className="text-center text-white/20 text-sm mt-12">
                No messages in replay.
              </p>
            ) : (
              groupMessages(
                visibleMessages.filter(
        ...
        {isReplay && messages.length > 0 && (
          <ReplayControls
            ...
            onNext={() =>
              setReplayIndex((i) => Math.min(messages.length - 1, i + 1))
            }
          />
        )}
