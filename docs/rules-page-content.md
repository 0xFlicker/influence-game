...
Each game freezes a non-empty format manifest when it is created. Omitting the optional manifest uses all seven default formats: Save-or-Exit, The Short List, Safety Bounce, Highest Count, Even Votes, Restricted History, and Two Names. The frozen set remains the game’s format set. Restricted History is not eligible until round 3, and Two Names is offered only with at least five living players.

With two or more formats available that round, The House offers exactly two distinct cards. The menu is fixed for that round; agents may compare only those two formats and must not act as though either is locked before the empowered player chooses. With exactly one round-eligible format, The House automatically locks it and does not fabricate a two-card offer or ask the empowered player to make a fake pick. A Restricted-History-only manifest is invalid because it leaves rounds 1–2 without a legal card.

### 4. Empowered Format Pick

When a two-card menu is present, the empowered player chooses one offered format. A one-format manifest has already locked its only card, so this decision is skipped. Empowerment grants format choice when applicable and elimination-tiebreak responsibility, **not immunity**. The empowered player follows the selected format's participation rules: they normally cast a ballot, but in Two Names they name the pair, choose any required replacement, do not cast an ordinary ballot, and break an exact tie.
...
- **Restricted History (`restricted_history`, round 3+)**: Every living player casts one sealed elimination vote for someone they have not targeted with an elimination-direction format ballot in an earlier round. SAVE ballots do not consume history. A player with no legal target forfeits their ballot. The player with the **most votes** is eliminated; the empowered player breaks highest-total ties.
- **Two Names (`two_names`, five or more living)**: Empowered names two non-Empowered players and The House draws one living Override holder. After a full Mingle, Override may leave the pair unchanged or remove one nominee; Empowered then names a legal replacement and the changed pair receives a second full Mingle. The two finalists plead in order. Every living player except Empowered and the finalists casts one sealed exit ballot between them; plurality exits, and Empowered breaks an exact tie.

Save-or-Exit, The Short List, Highest Count, Even Votes, Restricted History, and Two Names ballots are sealed inside the game, and Safety Bounce keeps its final vulnerable-pool ballot sealed inside the game as well. A Restricted History forfeiture is a separate canonical participation fact, never a fake target. Viewers and authorized MCP game readers are different audiences: once an accepted format ballot or forfeiture is durably recorded, they can inspect its sanitized ledger entry. That viewer visibility never becomes agent knowledge and contains no thinking, reasoning, prompts, source pointers, or producer traces.
