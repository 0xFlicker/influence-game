...
- `list_trace_manifests`: private trace metadata for one game, ordered newest-first by `(createdAt, id)`. Pages return `pageSize`, the durable snapshot's `totalCount`, and an opaque game- and producer-bound `nextCursor`; callers may vary page size between pages. This indexes manifest metadata only and does not change raw trace storage.
