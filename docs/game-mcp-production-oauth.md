...
- `read_owned_match_narrative`: token-efficient grouped narrative for a participating owner (default `strategic` + `compact`). Composes authorized dialogue with owned-seat strategy (and optional full cognition). Exact `decisionId` joins when stamped; otherwise honest inferred/uncorrelated correlation. **Not board-fact authority.** Members carry `authority: transcript | cognition` only — never reasoning dumps or private traces. Producer credentials do **not** silent-widen non-owned cognition on this tool.
...
- `read_producer_match_narrative`: token-efficient grouped narrative for producers (default `strategic` + `compact`). Full product dialogue scopes (public/system/mingle/whisper/huddle under capture-safe rules) plus all player/juror thinking/strategy. No ownership required. Does **not** embed private-trace bodies or reasoning dumps in members. `games:read` alone does not grant this tool.
