...
Authorization still issues exactly one grant scope. When provided, the requested `resource` selects the profile: `/mcp` grants `games`, and `/mcp/producer` grants `mcp`. For MCP App hosts that omit OAuth Resource Indicators, a single requested scope infers the matching canonical resource; mixed `games mcp` authorization requests still require `resource` so the grant is unambiguous. Authorization-code token exchange may also omit `resource`; the server uses the resource already bound to the authorization code. Requests are rejected if the registered/requested scope set includes unsupported scopes or does not include the selected resource profile's scope. Authorization codes and access tokens persist only `games` or `mcp`, never a mixed scope set.
