...
Use the deterministic fixture gate for structural promotion evidence. If one concrete context decision remains unresolved, use the private [real-thread context evaluator](prompt-thread-context-evaluation.md) to replay an authorized durable thread through isolated policy revisions and blind review. Its source validation and fake-provider tests are free; its curator and panel commands require separate immutable approvals and must never be launched as an implementation-agent completion step. Use a bounded full-game simulation only for the remaining integration/watchability question. A real-thread win is case-specific, and a full-game impression is not controlled causal evidence.
