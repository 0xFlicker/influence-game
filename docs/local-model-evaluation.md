...
Every huddle outcome in one sorted House schedule carries its one-based schedule position as the logical-call ordinal. This keeps accepted-result coordinates distinct and makes replay stable. House controls also validate the provider and accepted representations separately when nullable provider `thinking` normalizes to an omitted accepted field; replay accepts that canonical omission without another dispatch but rejects explicit null and other noncanonical stored values.
...
- Default games freeze all six registered formats: Save-or-Exit, The Short List, Safety Bounce, Highest Count, Even Votes, and Restricted History. Restricted History is admitted only from round 3. Rounds with two available cards contain `FORMAT MENU`, `FORMAT LOCKED`, and `FORMAT RESOLVE`, with no standard-round power action or Council elimination.
