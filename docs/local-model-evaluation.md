...
Hosted OpenAI game runs request the Flex service tier by default, including `simulate:api` durable games. Use `--standard` (or `--no-flex`) to request OpenAI's normal `auto` lane instead. The selected request tier is persisted in the game configuration; completion metadata records both call counts and token usage grouped by the actual `service_tier` returned by OpenAI, so a Flex-to-auto fallback is visible and priced at the effective rate. Flex responses use OpenAI's published Batch-rate equivalent; auto/default responses use the standard rate. Non-OpenAI providers neither receive a service-tier request nor receive a Flex discount.

Producer/admin durable-run inspection exposes the requested tier, aggregate effective-tier usage, and whether fallback occurred. Public game detail responses keep these internal accounting diagnostics private. Existing game configuration, checkpoint, result, and provider-spend JSON fields carry the metadata, so this behavior does not require a database migration.
