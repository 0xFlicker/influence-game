...
Note (as of 2026-07-26 checkpoint continuity recovery): supported startup recovery also hydrates versioned private **player continuity capsules** (Strategy Thread, reflection, notes, relationships, round history, power-action memory, recent decision receipts, packet-revision counter) for every active player at the sealed boundary. Capsules are checkpoint-bound recovery inputs only — not MemoryStore truth, not canonical projection, not transcript prose. Missing, duplicate, extra, identity-mismatched, malformed, or unsupported-version capsules fail closed. House continuity is conditional on the sealed checkpoint-time requirement (`disabled` / `awaiting_first_valid_update` / `required`): intentional absence is non-blocking when the contract allows it; required missing/malformed House continuity blocks recovery admission and passport readiness. Hydration passport stamps remain content-free structural diagnostics.
