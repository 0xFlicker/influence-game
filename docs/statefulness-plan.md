...
Note (as of 2026-07-26 format phase-entry recovery): startup recovery also supports the four format-kernel **phase-entry** coordinates — `format_menu`, `format_pick`, `format_mingle`, and `format_resolve` — when current-round canonical prerequisites validate. Menu/selection/pressure are reconstructed only from `format.menu_offered` / `format.selected` (never transcript prose). Mingle-inbox replay is session-scoped: Mingle I for menu/pick, discarded at format mingle entry (handler clears it), and Format Mingle only at format resolve. This is still not mid-action recovery: interruption during pick model calls, Mingle turns, sealed ballot collection, Safety Bounce pointer chains, or format resolution remains fail-closed unless a later complete phase-entry boundary exists.
