...
Note (as of 2026-08-04 social-cost cutover): startup recovery supports the four format-kernel **phase-entry** coordinates — `format_menu`, `format_pick`, `format_mingle`, and `format_resolve` — when current-round canonical prerequisites validate. Menu/selection/pressure are reconstructed only from `format.menu_offered` / `format.selected` (never transcript prose). The retired pre-format `mingle_i` and `pre_vote_huddle` coordinates remain inspectable in historical checkpoint data but fail closed for startup recovery. Format Mingle inbox replay is available only at format resolve. This is still not mid-action recovery: interruption during pick model calls, Mingle turns, alliance actions/huddles, sealed ballot collection, Safety Bounce pointer chains, or format resolution remains fail-closed unless a later complete phase-entry boundary exists.

Note (current capsule v2 contract): supported startup recovery hydrates versioned private **player continuity capsules** containing compact-strategy lifecycle, baseline, ordered deltas, prior reconciliation epoch when required, engine revision, notes, relationships, round history, and power-action memory for every active player at the sealed boundary. Capsules are checkpoint-bound recovery inputs only — not MemoryStore truth, not canonical projection, and never reconstructed from transcript prose. Missing, duplicate, extra, identity-mismatched, malformed, or unsupported/older-version capsules fail closed. House continuity is conditional on the sealed checkpoint-time requirement (`disabled` / `awaiting_first_valid_update` / `required`): intentional absence is non-blocking when the contract allows it; required missing/malformed House continuity blocks recovery admission and passport readiness. Hydration passport stamps remain content-free structural diagnostics.
