...
- KTD9. **Let only the active startup owner lift the pin.** If runtime startup fails before play, cleanup first closes the matching active owner epoch. It may return the game to `waiting`, remove competition-rating snapshots, and immediately reproject owned seats only when that compare-and-set affects exactly one owner row. A stale failure attempt returns a typed conflict and changes nothing. Suspended recovery never follows the profile because play already began.
...
5. **Failure:** any reconciliation, revision, local avatar-audit, or snapshot write failure rolls back the owning top-level transaction. External avatar generation remains asynchronous. A post-claim runtime startup failure may delete attempted snapshots, reproject owned seats, and return the game to waiting only after closing exactly one matching active owner epoch in the same compensating transaction.
