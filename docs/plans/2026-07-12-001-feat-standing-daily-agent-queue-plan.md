...
- **KTD8 — Reuse the canonical Agent editor.** The global provider owns eligibility/timing; owners without an Agent leave the prompt for the full-page Agent editor with a typed Daily Free return flow. The create request is idempotent, so a retry after enrollment failure reuses the created Agent rather than duplicating it.
...
The web provider waits for app authentication, active season, agents, membership state, and the root invite gate before arming the three-second timer. Zero-agent creation routes to the canonical full-page Agent editor with `flow=daily_free`; a successful create enrolls the new Agent and returns to `/games/free`, while cancel returns without creating anything. Client navigation preserves session dismissal because the provider sits above route content; a full reload reconstructs provider memory. This slice does not add an app-wide overlay registry.
