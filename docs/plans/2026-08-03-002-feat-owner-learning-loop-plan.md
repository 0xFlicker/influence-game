...
- R12. One review may reserve at most four logical model-call ordinals and perform at most three targeted moment dives across its entire lifetime, including explicit retries. The bounded Flex capacity transmissions and one standard fallback remain inside the current ordinal and do not consume additional logical-call budget.
...
5. **`agent_learning_review_calls`**
   ...
   - The reservation row is inserted before the logical call begins. Dispatch intent is persisted before every physical transmission; a returned terminal outcome is persisted before backoff or fallback continuation. Completion updates only that logical ordinal.
...
11. Enforce the 32,000 estimated-input-token ceiling with the shared deterministic character-based estimator over the complete serialized request input, including instructions and evidence, plus a fixed documented allowance for response-schema/envelope overhead. Evidence construction, provider admission, and tests use the same calculation. If compact narratives exceed the remaining budget, allocate a deterministic per-game share, retain canonical summaries and candidate IDs, and return explicit omitted-item counts. Do not silently drop one selected game or one authority lane.
...
- The worker heartbeats during provider I/O and checkpoint commits use compare-and-swap conditions on review ID, lease token hash, call ordinal, and prior checkpoint digest. Stale workers cannot advance state or complete a call row. In the initial single-worker deployment, internal supersede/shutdown aborts the local controller directly. Before multiple worker processes or API replicas may claim reviews, every heartbeat and transport-observer write must treat a failed active-lease compare-and-swap as a stop signal, abort the local fetch/backoff controller, and prevent later transmissions while retaining only usage already returned.
- Before every logical provider call, insert the next immutable call reservation and increment the lifetime counter transactionally. SDK `maxRetries` is zero. The engine transport may issue three total Flex transmissions followed by one identical `auto` transmission inside that ordinal. The worker persists dispatch intent before each transmission, persists each terminal outcome before continuation, and continues lease heartbeats during backoff. If the lease expires while the latest call remains `reserved`, a new worker may reclaim that same ordinal only after verifying its checkpoint and input/policy hash.
...
- Safe failure codes include `provider_capacity_exhausted`, `provider_timeout`, `provider_error`, `invalid_structured_output`, `tier_mismatch`, `input_budget_exceeded`, `output_budget_exhausted`, `logical_call_budget_exhausted`, `evidence_unavailable`, and `worker_interrupted`. Three Flex 429s followed by an auto-tier 429 map to retryable `provider_capacity_exhausted`. A response reported incomplete because `max_output_tokens` was reached maps to retryable `output_budget_exhausted`, discards the partial structured result, persists its usage/cost receipt, and retains only the last validated checkpoint. Arbitrary provider error bodies are neither stored nor shown.
...
Add seven protocol adapters over the same service and DTOs:
...
6. Build compact narratives for every selected game and include current `strategyStyle` for Strategy Health Check under the deterministic 32,000 estimated-input-token budget. Use one shared character-based estimator over the complete serialized request, including instructions and evidence, plus the documented fixed response-schema/envelope overhead allowance; expose omitted-item counts.
...
2. In the paid-start transaction, lazily insert then lock/re-read the entitlement row and atomically recheck/consume owner credit and the rolling 24-hour allowance with review creation. Reserve every later logical call and dive before provider I/O; safely reclaim only expired still-reserved ordinals with matching checkpoint and input/policy hash, and compare-and-swap the versioned local checkpoint after every validated response.
...
8. Register DB-ready startup, abort-aware shutdown, global claim behavior, and transport-receipt-aware recovery in the API without creating a general workflow framework. The initial single-worker process aborts local provider work on internal supersede/shutdown. Before multiple workers/replicas are enabled, a failed active-lease compare-and-swap must abort that worker's local request/backoff controller and prevent every later physical transmission or checkpoint write.
...
- No-change can complete on logical call one; three dives plus scan complete within four logical calls. Evidence-rich and Strategy Health Check fixtures retain their track; awaiting-evidence creates no review and issues zero calls. Covers AE5, AE7.
...
- In multi-worker fixtures, lease loss during fetch or backoff aborts the local controller and prevents all later transmissions and checkpoint writes; any already-returned usage receipt remains immutable.
