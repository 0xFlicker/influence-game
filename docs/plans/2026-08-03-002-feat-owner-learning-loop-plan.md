...
- **Spend policy:** A completed Daily Free ranked game can refill its owner's review-credit balance to one, and credits never stack. One paid review may dispatch per owner per rolling 24 hours. Credit and the 24-hour allowance are consumed at first provider dispatch, not during model-free preflight or review creation. Use `openai:gpt-5.6-luna` on Flex, never fall back to another tier, cap one review at four outbound model attempts and three moment dives, cap each response at 2,000 output tokens, and record token usage plus cost provenance (`actual | estimated | unavailable`) per call. There is no fixed dollar ceiling.
...
- **Use one owner-wide refillable credit with a rolling dispatch limit.** (session-settled: user-directed — chosen over one opportunity per agent or banked per-game runs so an owner can spend earned access on the agent that most needs review.) Each completed qualifying game can refill an empty owner balance to one, credits never stack, and one paid review may dispatch per owner per rolling 24 hours. A review targets one owned Profile and one to three selected current-revision games; old games may be selected again until that Profile changes revision. Governs R1-R4, R10, R15.
- **Allow only one unresolved review per owner.** (session-settled: user-directed — chosen so web and MCP always converge on the same work.) A new review cannot open until the existing review is applied, linked to a manual update, explicitly declined, automatically closed as no-change, cancelled while unfinished, or superseded by an unrelated update to the reviewed agent. Every review remains addressable by ID and owner-authorized after resolution. Governs R7, R17-R22.
...
- **Run reviews as durable low-cost work.** (session-settled: user-approved — chosen over a request-bound standard-tier call.) The waiting experience supports persisted Flex work and avoids request-bound duplicate spend. Governs R7-R9, R15.
...
- R3. Previously analyzed games remain selectable on the same current revision and display a derived `Previously analyzed` marker after a successfully completed analysis, including no-change. Failed or pre-completion-cancelled reviews do not mark their games.
...
- R29. One paid review may dispatch per owner per rolling 24 hours. Credit and the rolling allowance are reserved against concurrent starts but consumed only at first provider dispatch; cancelling before dispatch releases both, while any dispatched request consumes both even if the review later fails or is cancelled.
...
- R6. The web flow must preserve the repository-owned entry, investigation, and recommendations reference frames, with deterministic evidence as the primary workspace and analysis progress as secondary context. Agent switching, rolling-limit, generation-disabled, pre/post-dispatch cancel consequences, failure, no-change, Strategy Health Check, declined, cancelled, superseded, resolved, and existing-open-review extensions must use the same visual language.
...
- R8. Progress must report persisted stages such as evidence loading, narrative scanning, moment investigation, and recommendation drafting. Visual easing is permitted within the active stage, but the UI must not claim a numeric percentage or completion time it cannot know.
- R9. Flex capacity or model failure must preserve deterministic evidence and explain the failure without silently changing model or service tier. A retry must be offered only when the failure is retryable and lifetime attempt budget remains; otherwise the review must explain that its attempt budget is exhausted.
...
- R12. One review may issue at most four outbound model requests and perform at most three targeted moment dives across its entire lifetime, including retries.
...
- R33. Terminal resolution must distinguish `applied | manual_update | declined | no_change | cancelled | superseded`. Cancel is valid only for unfinished `queued | running | failed` work; decline is valid only for a completed ready recommendation and records that the generated proposal was not accepted.

**Web and MCP continuity**

- R20. Web and MCP must address the same singleton open review, credit and eligible-input read model, progress, evidence, outputs, resolution, and apply state rather than generating surface-specific copies. REST and MCP review reads must reauthorize the owner before returning existence, state, or counts.
- R21. MCP must let an authorized assistant list the owner's open reviews (zero or one), list eligible review inputs, start or resume the singleton review with one Profile and one to three game IDs, read a review by ID, retry it, apply its exact persisted strategy proposal, resolve a ready recommendation as declined, and cancel unfinished work under the same ownership, budget, idempotency, and revision rules as web. Mutating tools require `agents:write` in addition to the read scopes.
...
- R23. The product must record content-free funnel and operational signals for prompt impressions, dismissals, starts, evidence-track selection, persisted stages, completion, failure, retries, distinct cancellation/decline/supersede resolutions, token and cost usage, MCP connection movement, recommendation views, manual-edit handoffs, applies, and subsequent Daily Free play.
...
- F2. **Run and wait for a review**
  ...
  - **Actors:** A1, A2
  - **Steps:** Deterministic evidence renders and preflight selects evidence-rich, awaiting-evidence, or Strategy Health Check handling. Awaiting-evidence creates no review and spends nothing. A paid track reserves admission, consumes credit and the rolling allowance at first provider dispatch, runs the durable bounded harness, and advances the interface only when persisted stages advance.
  - **Outcome:** The singleton review waits honestly, reaches a completed recommendation set, or enters an understandable retry/cancel state.
...
- F5. **Continue through MCP**
  ...
  - **Actors:** A1, A2, A3
  - **Steps:** An unconnected owner enters the existing Games MCP setup and the browser returns to the same review after setup. The assistant lists the owner's zero-or-one open reviews, reads it by ID, uses existing authorized detail tools for follow-up, and then retries, applies, declines, links a manual update, or cancels unfinished work. A web-started review remains discoverable after the browser closes; an MCP-started review appears on the web resolver page.
...
- F6. **Refill credit and preserve review history**
  ...
  - **Actors:** A1, A2
  - **Steps:** Influence preserves completed reviews, derives `Previously analyzed` from successful review-game joins, and refills an empty owner balance to one. At paid dispatch it advances the owner cutoff through the latest qualifying completion so games completed while the balance was full do not become banked credits.
...
- AE4. **Covers R7, R12, R15.** Given a review is running or complete, when the owner refreshes, navigates away, retries, or enters from another surface, then Influence resumes the same review ID and lifetime attempt/dive counters do not reset. A concurrent start for any owned Agent Profile returns the existing open review instead of creating another.
- AE5. **Covers R2, R10-R14.** Given eight eligible games on one current revision, when the owner selects three and starts a review, then deterministic aggregates and AI context cover exactly those three, the investigator issues no more than four model requests, performs no more than three dives, and returns no more than three recommendations.
...
- AE8. **Covers R9, R12.** Given Flex capacity fails after deterministic facts load, when the job stops, then facts remain visible, failure is understandable, retry is available if lifetime attempt budget remains, and no standard-tier request is issued.
...
- AE11. **Covers R7, R20-R22, R33.** Given a web review is investigating and the browser closes, when the owner later connects MCP and lists open reviews, then the assistant receives that same review ID and can read it. Mutating tools require `agents:write`; another owner receives neither review data nor an existence signal. The assistant may cancel unfinished work or resolve a ready recommendation as declined by ID, and the reverse MCP-to-web flow reaches the canonical owned review.
- AE12. **Covers R23-R25, R33.** Given the owner completes, cancels, declines, supersedes, manually updates from, or applies a review, when analytics are inspected, then each truthful outcome is distinct without game-authored or generated event bodies and later Daily Free results remain attributed to game-effective revisions without causal language.
...
- AE14. **Covers R3, R15.** Given a game was part of a successfully completed ready or no-change review, when the owner later selects games for the same current revision, then that game remains selectable and is marked `Previously analyzed`. A failed or cancelled-before-completion attempt does not add the marker.
- AE15. **Covers R1-R4, R29.** Given an owner with one credit and no paid dispatch in the prior 24 hours, when a review is created and cancelled before its first provider dispatch, then the credit and rolling allowance remain available. Once a request dispatches, later failure or cancellation does not restore either, and the next qualifying completion may refill only the now-empty balance.
- AE16. **Covers R17-R19, R33.** Given a ready recommendation, viewing it leaves the review open. Exact apply resolves `applied`; an ordinary update with `sourceReviewId` resolves `manual_update`; `Keep current strategy` resolves `declined`; and no-change resolves automatically. Cancel is rejected for the ready review.
...
- AE21. **Covers R6, R29, R33.** Given unfinished review work, when Cancel is available before first dispatch, then the UI says cancellation preserves credit and rolling allowance. After first dispatch, it says both are already used and cancellation only stops unfinished work.
...
- One owner cannot hold more than one unresolved review, cannot hold more than one credit, and cannot dispatch more than one paid review in a rolling 24-hour window.
...
- Durable Flex execution, evidence reuse, failure, and retry.
- Evidence-linked recommendations and owner-approved strategy-only application.
- One owner-wide unresolved review shared across web and MCP, canonical review-ID URLs, and distinct apply/manual/decline/no-change/cancel/supersede behavior.
...
Planning preserves the confirmed product contract. V1 admits only Daily Free ranked play behind a versioned policy seam. One owner-wide credit never stacks, may be spent on any owned Profile with current-revision evidence, and is consumed with the rolling 24-hour allowance only at first provider dispatch. One unresolved review is shared by web and MCP and remains addressable by ID. Strategy Health Check is the only repeated-early-exit product name and enforces diagnosis and recommendation-proof contracts. Terminal outcomes distinguish apply, linked manual update, decline, automatic no-change, unfinished cancel, and unrelated-update supersede. R27 and AE13 retain the admin requirement to track cost, generated recommendations, and exact proposal acceptance. The previously proposed fixed dollar ceiling remains removed; credit admission, rolling dispatch limit, bounded turns/dives/tokens, Flex-only execution, per-call cost provenance, and admin visibility are the spend controls.
...
| Owner entitlement | One derived credit backed by any qualifying completion after an owner watermark; first provider dispatch advances the watermark through the latest qualifying completion and records the rolling-24-hour timestamp | The token bucket never exceeds one, does not require a game-completion critical-path write, and remains fungible across owned Profiles. Games completed while credit was already full do not bank future paid runs. |
| Review identity | Server-minted immutable review ID bound to owner + Agent Profile + current analytical revision + ordered one-to-three-game fingerprint + policy/evidence/reviewer versions | Start freezes the owner's explicit selection. A client idempotency key prevents duplicate starts without forbidding later reuse of the same game set. |
...
| Review model | `openai:gpt-5.6-luna`, requested `service_tier: "flex"`, `store: false`, SDK retries disabled | Luna is the catalog's cost-sensitive high-volume model; disabling fallback and client retries makes the budget enforceable. |
| Spend bounds | One owner credit, one paid dispatch per rolling 24 hours, 32,000 input tokens per request, 2,000 output tokens per request, four lifetime requests, and three lifetime dives | Credit and the rolling limit are consumed together at first provider dispatch. An immutable per-call ledger preserves actual/estimated/unavailable cost provenance. |
...
1. **`agent_learning_review_entitlements`**
   - One row per owner with the last consumed qualifying-completion watermark `(completed_at, game_id)`, last paid provider-dispatch timestamp, last surfaced threshold, and dismissal watermark.
   ...
   - Any start/dispatch path first lazily inserts the owner row with `ON CONFLICT DO NOTHING`, then locks and re-reads it. This gives a first-time web deep link or MCP start the same row-backed serialization without adding a game-completion write.
   - At first provider dispatch, one transaction repeats the lazy insert, locks and re-reads this row, rechecks the owner-wide unresolved review and rolling 24-hour allowance, verifies a credit exists, advances the consumed watermark through the latest then-visible qualifying completion, records dispatch time, and marks the first call dispatched.
...
3. **`agent_learning_reviews`**
   - Immutable key material: owner, Profile, reviewed current revision, ordered one-to-three-game fingerprint, start idempotency key, eligibility policy, analysis track, reviewer/evidence/prompt/schema versions, selected model/tier.
   - Orthogonal state:
     ...
     - resolution: nullable `applied | manual_update | declined | no_change | cancelled | superseded` plus `resolved_at`; a null resolution is the owner-wide open review.
   - Budget/worker state: lifetime outbound-attempt count, lifetime dive count, lease hash/expiry, safe failure code, retryable flag, and stage timestamps.
   - Versioned local checkpoint: selected moment order, next-moment cursor, provisional themes, accumulated validated findings, last completed stage, and the prompt/schema digests used to validate it. It contains validated generated state and typed refs, never raw provider bodies.
   ...
   - A partial unique index on `owner_user_id WHERE resolved_at IS NULL` enforces at most one unresolved review per owner across every Agent Profile. Apply, linked manual update, decline, no-change completion, unfinished cancel, and supersede write resolution in their successful transaction.
...
4. **`agent_learning_review_games`**
   ...
   - Successfully completed ready/no-change reviews make these joins the source for the derived `Previously analyzed` marker. Failed or cancelled-before-completion reviews do not contribute.

5. **`agent_learning_review_calls`**
   - Immutable call ledger unique on `(review_id, ordinal)` with `reserved | dispatched | succeeded | failed | ambiguous` state, stage, input/policy hashes, provider request ID, requested/effective tier, token receipt, latency, safe failure code, and timestamps.
   - Cost follows the existing provider accounting contract: `costSource`, `actualCostMicrousd`, `estimatedCostMicrousd`, `pricingSourceId`, `rateCardVersion`, and `pricedAt`. A catalog calculation is an estimate, never relabeled actual.
   - The reservation row is inserted before provider I/O. Completion updates only that ordinal under the active review lease; no provider response body is persisted.
...
8. **`agent_learning_events`**
   - Append-only content-free events: `prompt_impression`, `prompt_dismissed`, `review_started`, `analysis_track_selected`, `credit_consumed`, `stage_reached`, `review_failed`, `review_retried`, `review_cancelled`, `review_declined`, `review_superseded`, `review_resolved`, `recommendations_viewed`, `manual_editor_opened`, `proposal_applied`, `mcp_offer_viewed`, and `mcp_connected`. Subsequent Daily Free play is derived from canonical competition receipts by game-effective revision rather than duplicating game truth in this ledger.
...
6. Run structural preflight before review creation. If it returns `awaiting_evidence`, return deterministic facts and guidance without inserting an unresolved review, reserving a provider call, or consuming credit/daily allowance. If live generation is disabled, return the same deterministic evidence plus a typed unavailable state with the same no-write/no-spend behavior. Otherwise lazily insert the owner entitlement row with `ON CONFLICT DO NOTHING`, lock and re-read it, and create the exact review/review-game set under the owner-wide open-review unique index.
7. Materialize or reuse deterministic game evidence lazily. Deterministic display and AI context both cover the exact one-to-three-game selection.
...
10. Mint candidate moment IDs from stable game/evidence version, anchor kind, stable source coordinate, and window version. A later request must prove the ID belongs to the review's candidate set; the model cannot invent IDs or tool arguments.
11. If compact narratives exceed the 32,000-token request ceiling, allocate a deterministic per-game budget, retain canonical summaries and candidate IDs, and return explicit omitted-item counts. Do not silently drop one selected game or one authority lane.
...
stateDiagram-v2
  ...
  queued --> resolved: cancel or supersede
  running --> resolved: cancel or supersede; late result discarded
  failed --> resolved: cancel or supersede
...
- Starting is idempotent on the owner/start-request key and serialized by the owner-wide unresolved-review constraint. Concurrent web/MCP starts for the same or different Agent Profiles return the one existing open review. Reusing the same games in a later resolved review remains allowed while the Profile stays on that revision.
...
- The worker heartbeats during provider I/O and checkpoint commits use compare-and-swap conditions on review ID, lease token hash, call ordinal, and prior checkpoint digest. Stale workers cannot advance state or complete a call row.
- Before every outbound provider request, insert the next immutable call reservation and increment the lifetime counter transactionally. For the first request only, the dispatch transaction repeats the entitlement-row `INSERT ... ON CONFLICT DO NOTHING`, locks and re-reads it, rechecks the derived credit and rolling 24-hour allowance, advances the credit watermark through the latest qualifying completion, records the dispatch timestamp, and marks the call `dispatched` immediately before network I/O. SDK `maxRetries` is zero. If the lease expires while the latest call remains `reserved`, a new worker may reclaim that same ordinal only after verifying its checkpoint and input digest, without incrementing attempt or dive counters. A process crash after dispatch leaves an ambiguous, consumed ordinal that is never replayed automatically and never restores the credit or rolling allowance.
- Recovery turns any expired `running` review with a dispatched call into `failed/worker_interrupted` and that call into `ambiguous`. The deterministic workspace remains available; only an explicit retry may reserve a new ordinal if budget remains.
- A retry clears only retryable failure fields, sets analysis status to queued, and resumes the exact versioned checkpoint/cursor. It never resets lifetime request/dives, reprocesses a validated moment, changes model/tier, or discards findings. Only the unfinished step may be attempted again if budget remains.
- Cancel is idempotent by review ID only for unresolved `queued | running | failed` work. It resolves as `cancelled` without mutating the Agent Profile; a running cancel invalidates its lease and rejects late provider/checkpoint finalization. Cancellation before first dispatch preserves credit and daily allowance, while a dispatched call and its cost remain consumed. Ready work rejects cancel.
- No-change resolves automatically. Exact apply resolves as `applied`. A manual web or MCP update resolves as `manual_update` only when it supplies the owned `sourceReviewId`. `Keep current strategy` resolves a ready review as `declined`; viewing alone does not resolve it.
...
- Safe failure codes include `flex_unavailable`, `provider_timeout`, `provider_error`, `invalid_structured_output`, `tier_mismatch`, `input_budget_exceeded`, `attempt_budget_exhausted`, `evidence_unavailable`, and `worker_interrupted`. Arbitrary provider error bodies are neither stored nor shown.
- The worker must verify the provider's effective service tier. Any non-Flex response fails as `tier_mismatch`; there is no standard-tier fallback.
- API startup begins the loop only after database readiness. Graceful shutdown stops claiming, aborts local provider waits, and either releases the lease safely or lets its short expiry recover. An environment switch may disable processing for tests, but enabled replica count cannot change global concurrency or provider policy.
...
- **Accounting:** Record per-request requested/effective tier, input/cached/output/reasoning tokens, provider request ID, latency, and cost provenance using the existing provider accounting contract. Derive review totals for web/admin display from immutable call rows; log only safe numeric/enum fields.
...
- `POST /reviews/:reviewId/resolve` — accept only `resolution: "declined"` for an unresolved ready recommendation, record `Keep current strategy`, and mutate no Agent Profile field.
- `POST /reviews/:reviewId/cancel` — idempotently cancel only unresolved queued/running/failed work, invalidate active worker finalization, resolve the singleton slot, and mutate no Agent Profile field.

The public owner DTO is the canonical application contract reused by the MCP adapter. It distinguishes analysis status, stage, track, and terminal resolution; derives apply disposition from application/current revision state; includes stable evidence/proof refs and safe failures; and never includes lease fields, prompt hashes, provider request IDs, cost formulas, source bodies, or admin-only owner identifiers. The web canonical route is `/dashboard/agents/[id]/review/[reviewId]`; it verifies that the authenticated owner owns both the path Profile and review and that they correspond. The agent-level `/review` route resolves the owner's open review and redirects to its canonical URL, or shows eligible inputs when none is open. Another owner receives the same unavailable response as a nonexistent review.
...
- `cancel_learning_review`

Authorization:

- Eligible-input listing, open-review listing, and read use `requiredScopes=[agents:read,games:read]`, `catalogBaselineScopes=[agents:read]`, and `clientEnvelopeScopes=[agents:read,games:read]` for the authenticated subject owner.
- Start/resume, retry, apply, resolve, and cancel use `requiredScopes=[agents:read,games:read,agents:write]`, the two read scopes as their catalog baseline, and all three scopes as their client envelope. This makes paid-work or durable-state step-up visible only from an already authorized review context.
...
- Eligible-input/open-review listing and read are `readOnly=true, idempotent=true`; start/resume and retry are `readOnly=false, destructive=false, idempotent=true`; apply, resolve, and cancel are `readOnly=false, destructive=true, idempotent=true`. Start/resume accepts one Profile and one to three game IDs and returns `created | resumed | existing_open_review | awaiting_evidence`, whether paid work was newly enqueued, current persisted stage, and remaining attempts/dives. Open-review listing returns at most one item in V1.
...
- `resolve_learning_review` accepts a review ID and resolves only an unresolved ready result as `declined`, after the assistant confirms the user wants to keep the current strategy.
- `cancel_learning_review` documentation limits cancel to queued/running/failed work; cancelling a running review prevents persisted finalization but cannot restore credit, rolling allowance, or cost after provider dispatch.
...
`apply_learning_review` is only for the exact persisted proposal. After deeper MCP analysis, a customized owner-directed update uses the existing `get_agent`/`update_agent` path with optional owned `sourceReviewId`, returns the ordinary mutation receipt, creates no review application, and resolves that review as `manual_update`. An update without `sourceReviewId` to the reviewed Profile atomically resolves it as `superseded`; an update to another Profile has no effect. MCP continuity comes entirely from list/read/start/retry/apply/resolve/cancel by review ID; it does not require browser URLs.
...
- eligibility-policy version, analysis track, analysis status/stage, resolution kind, failure code, retry/attempt/dive counts, reviewer version, model, requested/effective tier;
- input/cached/output/reasoning tokens, latency, and total cost separated into actual/estimated/unavailable provenance with preserved pricing-source/rate-card metadata;
...
- disposition `not_ready | awaiting_owner | applied | manual_update | declined | no_change | cancelled | superseded`, per-recommendation `accepted | not_accepted | not_applicable` derived from the applied proposal's immutable source IDs, resolution timestamp, prior/resulting revisions when applicable, and stored mutation receipt summary. Only `applied` accepts the exact generated proposal; `manual_update` is review-driven but not acceptance, `declined | cancelled | superseded` are not accepted, and `no_change` is not applicable.
...
- Reauthorize every list, preflight, start, read, retry, apply, resolve, cancel, and linked manual resolution; possession of a review, game, or moment ID is not authority.
...
- Treat all generated review prose as untrusted bounded plain text on owner and admin surfaces; never interpret model-authored markup or links.
...
- **API runtime:** Every enabled API replica may host the loop, but the DB advisory claim gate and unexpired-lease check enforce one global active review. Startup waits for DB readiness; shutdown stops claims and aborts/lets the current short lease expire.
- **MCP/OAuth:** Catalog visibility, paid-work/durable-state step-up, zero-or-one open-review listing, read/start/retry/apply/resolve/cancel invocation, HTTP authentication, and safe connection measurement all change. The conversion event is observational: a failed event write reports a safe operational error and cannot fail an otherwise valid MCP request.
...
- **Integrity audit before deployment:** Verify zero orphan children, recomputed review fingerprints match their ordered one-to-three-game joins, selected games belong to the reviewed current revision and use `trackType = free`, entitlement watermarks are monotonic, no owner has more than one unresolved review, no owner has two paid first dispatches inside 24 hours, no more than one unexpired active worker lease exists, lifetime counters match unique call/dive records, review cost totals equal immutable call receipts, and every application matches its review/fingerprint/prior/resulting revisions.
...
6. Add strict structured-result validation with the R34 per-field length limits and deterministic fingerprint helpers using normalized JSON and cryptographic hashing.
...
- Owner/start idempotency rejects duplicate starts while later review IDs may reuse the same game fingerprint on the unchanged revision.
...
- DTO parsers reject unknown evidence refs, generated-field length overflow, recommendation overflow, invalid Strategy Health Check proof/rubric combinations, non-`strategyStyle` proposals, and free-form event metadata.
...
6. Build compact narratives for every selected game and include current `strategyStyle` for Strategy Health Check under the deterministic 32,000-token budget; expose omitted-item counts.
7. Mint and validate stable moment IDs from exact durable coordinates; never use page-local narrative group IDs as durable evidence.
...
- Several completions while credit is full still derive one credit. Paid dispatch advances through the latest committed qualifying completion, while a later completion refills the empty balance. Covers AE15.
...
- Previously completed-analysis games remain selectable and marked; failed/pre-completion-cancelled joins do not mark. Covers AE14.
...
2. Reserve every outbound attempt and dive before provider I/O. At the first dispatch, repeat the lazy entitlement-row insert, lock and re-read it, then atomically recheck/consume owner credit and the rolling 24-hour allowance with the call transition; safely reclaim only expired still-reserved ordinals with matching checkpoint/input digest, and compare-and-swap the versioned local checkpoint after every validated response.
3. Add a review-specific Responses client pinned to Luna/Flex, `store: false`, 32,000 input tokens, 2,000 output tokens, and SDK `maxRetries: 0`. Do not call the general engine path that falls back from Flex to standard after 429s.
4. Implement separate versioned evidence-rich and Strategy Health Check policies inside the same four-turn ceiling, strict proof-shape/ref validation, explicit observation/interpretation/guidance fields, server-minted refs, and no-change behavior. Awaiting-evidence and generation-disabled paid tracks never create a review or enter the provider harness.
5. Validate effective tier and persist token usage plus actual/estimated/unavailable cost provenance per immutable call. Store only safe errors and provider request identifiers.
6. Keep generated review-specific findings in the checkpoint; cache only context-independent authorized moment-bundle preparation.
7. Register DB-ready startup, abort-aware shutdown, global claim behavior, expired-reservation/dispatched recovery, and cancellation-aware late-result rejection in the API without creating a general workflow framework.

**Patterns:** Reuse claim/lease/stale-finalize concepts from `packages/api/src/services/postgame-media-worker.ts:65-318`; reuse strict tier checking, `store: false`, and no-SDK-retry ideas from `packages/api/src/services/prompt-thread-provider-broker.ts:160-299` without importing its filesystem workspace; use `openai:gpt-5.6-luna` from `packages/engine/src/model-catalog.ts:175-178`.

**Test scenarios:**

- Concurrent starts for the same or different owned Agent Profiles return the one owner-wide open review; concurrent first dispatches consume one credit/rolling allowance; two worker IDs/API replicas still produce at most one unexpired active lease globally.
- Stale workers cannot finalize. An expired reserved call reuses the same ordinal/digest without another counter increment; an expired dispatched call becomes ambiguous/failed and is never replayed automatically.
...
- Crash before dispatch, after dispatch, after response, and during checkpoint cannot exceed four lifetime slots or double-count a call/cost receipt. Covers the reserved-call recovery rule explicitly.
- 429, timeout, invalid output, tier mismatch, and exhausted budget preserve deterministic evidence and never invoke standard tier. Covers AE8.
- Unknown moment/ref output, prompt injection text, generated-field length overflow, and recommendation overflow fail validation.
- No-change can complete on turn one; three dives plus scan complete within four requests. Evidence-rich and Strategy Health Check fixtures retain their track; awaiting-evidence creates no review and issues zero calls. Covers AE5, AE7.
...
- Cancelling queued/running/failed work resolves the owner slot exactly once; ready cancel is rejected. A late response after running cancellation records only its immutable call receipt and cannot write a checkpoint/result. Pre-dispatch cancel preserves admission; post-dispatch cancel does not. Covers AE15-AE16.
- Fake provider receipts aggregate cached/input/output/reasoning tokens and sourced cost; rate-card changes do not rewrite history and ambiguous cost is unavailable rather than zero. No paid/network model call occurs in tests.
...
**Goal:** Apply only the exact persisted `strategyStyle` proposal with current-revision protection, and resolve/cancel reviews without creating a second mutation authority.
...
7. Implement ready-state decline and unfinished cancel by owned review ID. Both mutate no Profile field, but retain distinct resolution/events and state validation.
...
- Application existence derives `applied`; the terminal resolution distinguishes linked manual update, decline, no-change, cancel, and supersede without a stale flag.
- Apply, linked manual update, decline, automatic no-change, unfinished cancel, and supersede each resolve once under races with a competing start; exactly one next open review can be created after commit. Covers AE16.
...
1. Add eligible-input, impression, dismissal, preflight, zero-or-one open-review list, singleton start/resume, review-by-ID read, retry, viewed, apply, ready resolve, and unfinished cancel routes defined above.
2. Keep route handlers thin: authenticate, parse strict input, call the protocol-neutral service, map stable domain errors.
...
- Open-review listing returns zero or one. Retry, apply, ready decline, linked manual resolution, supersede, and unfinished cancel enforce ownership/state/budgets/fingerprint/revision freshness and unblock exactly one later start.
...
2. Treat the repository-owned design reference and three HTML frames as the visual acceptance authority. Build the approved entry/analyzing/ready hierarchy first, then extend the same language to game selection, awaiting-evidence, Strategy Health Check, failure, declined, cancelled, superseded, resolved, and existing-open-review states.
...
6. Render recommendations as evidence-linked editorial cards. Activating evidence selects its game, focuses/highlights the exact in-page moment, and retains Back to recommendation. Render Strategy Health Check support labels from structured proof, show the exact strategy diff, and keep manual edit/apply/keep-current actions legible; show cancel only for unfinished work.
...
12. Show the review's provider-admission state beside every unfinished Cancel action. Before first dispatch, state that cancellation preserves credit and the rolling allowance. After dispatch, state that both are used and cancellation only stops unfinished review work.
...
- Failure preserves facts and offers retry only when allowed plus cancel. Superseded explains that newer owner work won. Applied/manual/declined/no-change/cancelled/superseded resolutions remain legible. Covers AE8, AE10, AE16.
- Initial load/start/retry/apply/decline/cancel/manual-return pending and failure states disable duplicates; lost mutation responses reconcile from persisted state before another action is offered.
- Rolling-limit preserves the deterministic workspace, shows the exact next-eligible time, disables start, and recovers after a fresh eligible read. Pre-dispatch and post-dispatch Cancel show their different credit/allowance consequences. Covers AE20-AE21.
...
1. Register the eight tools with exact input/output schemas and truthful read/side-effect annotations, including eligible-input and zero-or-one open-review listing plus resolve/cancel by review ID.
...
4. Add exact-diff/fresh-confirmation language to tool descriptions/rules and accept only review ID + fingerprint on apply. Resolve and cancel accept only review ID with their fixed state semantics; all mutating/paid-work calls reauthorize write scope.
...
7. Document/test custom manual MCP updates through existing `get_agent`/`update_agent` with optional owned `sourceReviewId`, distinct from exact proposal acceptance. Do not implement browser-URL handoff.
...
- Authenticated REST routes and real MCP HTTP/tool transport serialize equivalent canonical review identity/state/result/apply fields for one DB fixture; MCP-only follow-up affordances validate against target tool schemas and cannot escape the authorized source set.
- Web-start then browser-close is discoverable through MCP open-review listing; MCP-start is discoverable through the web resolver. The list returns zero or one, and every fetch/resolve/cancel by ID rejects other-owner access without an existence signal. Covers AE11.
...
- Resolve declines only ready work. Cancel is idempotent only for unfinished work, creates no agent mutation, closes the singleton slot, and prevents late worker finalization.
- Tool descriptions require fresh confirmation and never claim server-verifiable conversational consent.
...
5. Document the Owner Learning Loop vocabulary, Daily Free eligibility/owner-credit seam, one-to-three-game current-revision selection, owner-wide singleton/resolution lifecycle, source authority, eight MCP tools/scopes, bounded model policy, Strategy Health Check proof contract, failure/retry/resolve/cancel behavior, visual reference, and operations runbook.
...
- A linked same-Profile manual edit resolves as manual update but does not mark the generated proposal accepted; a cross-Profile link is rejected; an unrelated same-Profile effective-input edit resolves as superseded. Declined/cancelled/superseded are not accepted and no-change is not applicable.
...
2. Focused DB tests for owner-credit watermark/rolling allowance, owner-wide singleton identity/resolution, reserved/dispatched lease recovery, retry/cancel/supersede concurrency, exact apply, analytics, and admin reads. Every DB-mutating test calls `setupTestDB()` and remains sequential inside its Bun process.
...
- Failure/retry preserves facts and budget; supersede preserves newer work; cancel/decline/manual/no-change/apply/supersede each resolve the singleton correctly.
- Initial load/start/retry/apply/decline/cancel response loss reconciles without duplicate action.
...
4. Repeat apply from web and prove idempotency; prove linked web/MCP same-Profile updates resolve as manual update with no application, a cross-Profile link is rejected before writes, an unlinked effective-input update supersedes, a presentation-only update does not, ready resolve declines, and unfinished cancel closes without mutating the agent.
5. Attempt start/retry/resolve/cancel with missing write scope, read/list with one missing read scope, insufficient client envelope, producer-only scope, and another owner; prove correct step-up/unavailable behavior and no counts or existence leak.
...
8. Assert tool guidance requires fresh confirmation, apply input has no approval boolean, MCP continuity uses list/read by ID rather than a URL, and the test does not claim the server proved a prior user utterance.
...
- attempts/dives/input/cached/output tokens, service tier, latency, and actual/estimated/unavailable cost provenance match the immutable call ledger;
- Flex unavailability remains a failure/retry, never a fallback.
...
- [ ] One owner has at most one credit, one paid dispatch per rolling 24 hours, and one unresolved review; web and MCP list/read/apply/resolve/cancel it by ID and preserve ownership privacy.
...
- [ ] Admin can inspect cost, generated recommendations/proposal, and applied outcome.
...
- [ ] Lifetime attempt/dive budgets survive retry, crash, refresh, and surface changes.
- [ ] No code path silently changes model or service tier.
...
- [ ] Apply, linked manual update, decline, no-change, unfinished cancel, and supersede have distinct atomic terminal outcomes; only apply accepts the generated proposal.
...
- [ ] Admin diagnostics identify failed/stuck reviews, safe failure codes, token usage, sourced cost, and application outcome.
- [ ] Deployment enablement, owner-wide singleton resolution, worker ownership, reserved/dispatched lease recovery, and rollback procedure are documented.
...
| Retry multiplies spend | Reserve lifetime attempts before I/O, disable SDK retries, preserve counters/checkpoints | Crash/retry concurrency tests |
| A pre-dispatch crash wastes an attempt | Reclaim only the same expired reserved ordinal after checkpoint/input-digest verification | Reserved-call fake-clock test |
...
| A completion or concurrent start races admission | Freeze exact reauthorized game IDs under the singleton constraint; consume the owner credit watermark and rolling allowance atomically at first dispatch | Selection/start/dispatch race tests + watermark audit |
| A crash leaves provider delivery uncertain | Persist dispatched calls as ambiguous, never auto-replay them, and require explicit retry within the lifetime budget | Four crash-boundary tests |
| Flex path silently upgrades | Review-specific provider client, effective-tier validation, no fallback | 429/tier-mismatch tests |
...
- `packages/api/src/services/avatar-generation.ts:782-826` — existing service-owned quota/idempotency pattern; owner learning uses a separate owner credit watermark plus rolling dispatch timestamp.
...
- `packages/api/src/services/prompt-thread-provider-broker.ts:160-299` — strict effective tier, `store: false`, and no-retry provider behavior to reuse without its filesystem workspace.
