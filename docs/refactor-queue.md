...
- Durability and recovery: `HouseNarrativeContinuityV2` checkpoints recent public beats and the private notebook together before buffered viewer delivery. Version-1 House capsules have no compatibility parser; active games must drain before deployment. Provider refusal or exhaustion emits no fabricated summary and preserves the previous notebook under the bounded pending-delta policy.
