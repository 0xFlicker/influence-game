...
```
INTRODUCTION -> LOBBY -> VOTE -> FORMAT_MENU -> FORMAT_PICK -> FORMAT_MINGLE -> FORMAT_RESOLVE
```
...
Start the `gameId` returned by the draw with `POST /api/free-queue/start?gameId=<game-id>`; do not rely on a newest-game lookup when an operator already has the exact draw receipt. Start checks roster and provider/model readiness before claiming a run owner. A readiness failure therefore leaves the game waiting with no owner to clean up. If synchronous runner startup fails after the claim, the API revokes that claim, restores the waiting roster when safe, and a retry should target the same `gameId`. A `repair_required` cleanup response needs operator investigation rather than blind retries.

The production scheduler passes its exact scheduled time to `cron-free-game-draw`, which derives `daily-free:schedule:<scheduled-time>` and sends the same key to staging and production. For a missed scheduled draw, invoke the Lambda with the original `scheduledTime`. For a deliberate manual draw, pass a unique `drawRequestKey`, or run `scripts/free-game-draw.sh`; the script prints its generated key. Reuse that exact value after an ambiguous timeout. Even if an operator accidentally supplies a different key, the waiting-game convergence rule returns the already-created roster until it starts.
...
worker is safe because the existing per-game owner epoch fences commits; use a
different `PORT` when both run on the same machine. Never point an ad-hoc local
game worker at staging or production data.
...
The visual agent-context reader checks `game_run_owners` and the exact committed execution heads/cursor both before and after loading images. Visual scene `boundarySequence` is compared to the durable **turn sequence**, not the canonical event sequence. A `VisualTurnNotReady` error must hold gameplay for scene preparation or recovery; it must not be converted into a text-only agent turn. Lifecycle installation and operator recovery routing are still pending.
