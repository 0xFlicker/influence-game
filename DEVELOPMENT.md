...
The current Streamable HTTP MCP is documented in `docs/game-mcp-production-oauth.md` and has two resource profiles. `/mcp` uses `scope=games` for user-facing rules discovery, owned-agent management, supported pre-match enrollment, and accessible game inspection. It must not expose private trace tools, producer visibility, active-match actions, or moderator controls. `/mcp/producer` uses `scope=mcp` plus the current `mcp` role for producer/global inspection and explicit private trace tools. User-facing agent responses may include account-level free-track ELO provenance plus per-agent games/wins, but must not claim true per-agent ELO until that source exists.
