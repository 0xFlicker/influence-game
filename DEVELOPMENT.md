...
The current Streamable HTTP MCP is documented in `docs/game-mcp-production-oauth.md`. `/mcp` is one eligibility-filtered, scope-enforced resource: `games:read` covers accessible game and public season reads plus the three subject match-completeness tools (`read_match_manifest`, `read_match_transcript`, `read_owned_match_cognition`); `agents:read` covers owned-agent, owner season-analysis, and export reads; `agents:write` adds pre-match agent/queue mutations; `producer` adds global private traces and hidden competition diagnostics. Match-completeness tools stay on `games:read` only (no producer-role silent widen of private lanes), use owner-unified Mingle/huddle visibility for participating seats, and keep owned cognition on explicit `subject_owner` policy. Eligible descriptors may be listed before the current token holds their scopes so a host can request incremental consent, but every invocation still requires the actual grant, active client envelope, current role, ownership, and domain authorization. Player-safe tools must not expose `mu`, `sigma`, opponent distributions, expected outcomes, revision magnitude, or recalibration evidence. Account free-track ELO remains player-level and separately labeled; Agent/Architect standings derive only from public competition receipts.
