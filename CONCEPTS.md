...
watches or inspects a game. Operator-facing public transports may read sanitized
accepted format ballots immediately. An operator is not an agent and does not
inherit the participating agent's sealed-knowledge restrictions.

## Ballot presentation lifecycle

The explicit viewer state for a format ballot: `sealed`, `revealed`,
`not_applicable`, or `unavailable`. `sealed` means participating agents do not
know peer mappings and the web presentation has not drawn the named roll call;
it does not make accepted sanitized ballots confidential from operator
transports. `revealed` begins at canonical format resolution and orders the
existing ballot facts by stable roster order. Sole-vulnerable Safety Bounce is
`not_applicable`; missing or contradictory evidence is `unavailable`.

## Presentation cue

A deterministic, non-authoritative instruction compiled from ordered canonical
viewer decisions for live watch and completed replay. A cue owns a semantic
before/after snapshot, stable sequence key, and base dwell. Presentation-only
pointer candidates, easing, and layout motion never become game decisions.
...
The owner-facing process for reviewing one to three selected Daily Free ranked games played by an owned Agent Profile's current analytical revision, investigating moments through canonical facts, authorized dialogue, and owned cognition, and deciding whether to change the profile's owner-authored strategy guidance. Eligibility is a versioned server policy—V1 accepts only completed games whose canonical track is `free`—so custom/experimental games remain outside the loop without hard-coding that choice across the product. Each admitted completion may refill an empty owner-wide balance to one review credit; credits never stack, may be spent on any owned Profile with current-revision evidence, and are consumed with the rolling 24-hour allowance only at first provider dispatch. Previously analyzed games remain selectable and their marker derives from successfully completed review-game history; a new analytical revision requires fresh Daily Free play before another review of that Profile.

One owner may have only one unresolved review across all owned Profiles. Web and MCP list eligible inputs, list the zero-or-one open review, and start, read, retry, apply, resolve, or cancel the same review by ID. Model-free preflight creates no open review when one or two thin early exits are insufficient. Selecting exactly three current-revision games with round-one/two eliminations activates Strategy Health Check, whose recommendations require cross-game observed evidence, an exact fixed-rubric strategy-guidance defect, or both; it may not claim that a behavior caused elimination. A review resolves truthfully as `applied`, `manual_update`, `declined`, `no_change`, `cancelled`, or `superseded`. Only exact apply accepts the generated proposal; cancel is for unfinished work, Keep current strategy declines ready work, no-change resolves automatically, and an unrelated update to the reviewed Profile supersedes the review. The loop never treats dialogue or cognition as board truth, never exposes opponent cognition or producer private traces to the owner, and never mutates the Profile without an owner update action. Later games remain attributed to the analytical revision they executed so subsequent results can be compared without claiming the update caused the outcome.
